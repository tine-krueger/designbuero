	...
	return (
		<div className={classes} {...attributes}>
			{images.map((image) => {
				...
				return link ? (
					<Link key={uid(image)} href={link.href} className={classNames(styles.link, childElementsClasses?.link, 'no-link')}>
						<CustomImage className={classNames(styles.image, childElementsClasses?.image)} objectFit={objectFit} priority={imagesHavePriority} {...rest} />
						{hoverOverlay && (
							<div className={styles.overlay}>
								<div className={styles['snippet-wrapper']}>
									<p className={classNames(styles.snippet, `c-t--${NGColor.white}`, 'font-style--l')}>{hoverOverlay}</p>
								</div>
							</div>
						)}
					</Link>
				) : (
					<CustomImage key={uid(image)} className={classNames(styles.image, childElementsClasses?.image)} objectFit={objectFit} priority={imagesHavePriority} {...rest} />
				)
			})}
			{children}
		</div>
